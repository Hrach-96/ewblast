	...
	@foreach($events as $res)
		<div class="row itemblock">
			<div class="col-lg-12">
				...
				<h2><a href="{{route('events.display',['id'=>$res->id,'lang'=>mainLang()])}}">{{$res['title_'.mainLang()]}}</a></h2>
				<p>{!! $res['content_'.mainLang()] !!}</p>
				<p><a href="{{route('events.display',['id'=>$res->id,'lang'=>mainLang()])}}">Կարդալ ավելին</a></p>
			</div>
		</div>
	@endforeach
</div>
